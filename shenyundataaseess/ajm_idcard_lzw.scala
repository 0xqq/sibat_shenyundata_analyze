...
/*  case class basicDev(gb_code: String, dev_id: String, dev_type: String, dev_name: String, dev_mac: String, dev_ip: String,
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("ajm_idcard_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("ajm_idcard_lzw")
      .getOrCreate()
...
/*    /** 文件basic_device_info.txt 的RDD形式转为DataFrame**/
    ...
    //val parquetDFajmorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/ajm_idcard/"+args(0))
    //对异常文件处理
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/ajm_idcard/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFajmorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        parquetDFajmorg = sqlContext.read.parquet(file_path)
      } catch {
    ...
    /* |-- serverReceiveTimestamp: long (nullable = true)
     ...
     |-- capTime: string (nullable = true)
     |-- deviceNum: string (nullable = true)
     |-- deviceSn: string (nullable = true)
     |-- gbNo: string (nullable = true)
     |-- cameraOneFace: string (nullable = true)
     |-- cameraTwoFace: string (nullable = true)
     |-- deviceAddr: string (nullable = true)*/

    /*************************************************   需求1   *****************************************************************/
    val originnum = parquetDFajmorg.count()
    val parquetDFajm =parquetDFajmorg.distinct().dropDuplicates()
    val distinctnum =parquetDFajm.count()

    parquetDFajm.createOrReplaceTempView("ajm_table")
    val distinctnull = sqlContext.sql("select * from ajm_table where gbNo !=''")

    val distinctnullnum = distinctnull.count()
    val onlinenum = sqlContext.sql("select distinct gbNo from ajm_table where gbNo !=''")
    val finalajm = sqlContext.sql("select distinct gbNo from ajm_table ,  deviceInfo_table where ajm_table.gbNo = deviceInfo_table.gb_code ")
    val noStationajm = sqlContext.sql("select distinct gbNo from ajm_table ,  deviceInfo_table where  ajm_table.gbNo = deviceInfo_table.gb_code and  deviceInfo_table.station_name=''")
    // val distinctStationajm = sqlContext.sql("select station_name,count(*) ")
    val fitdistinctStationajm = sqlContext.sql("select station_name,count(distinct gbNo) from ajm_table, deviceInfo_table where ajm_table.gbNo = deviceInfo_table.gb_code group by station_name")

    /** 需求1的一系列输出 **/
    //写在本地
/* val writer = new PrintWriter(new File("/user/zhangjing/GongAn_analyze/ajm_idcard/"+args(0)+"/divice_number.txt"))
    writer.write("全网匹配数: "+finalajm.count()+"\r\n")
    writer.write("全网在线数: "+onlinenum.count()+"\r\n")
    writer.write("去重数据比: "+((originnum-distinctnum)/originnum).toFloat.toString+"\r\n")
    writer.write("无站点设备数: "+noStationajm.count())
    writer.close()
    //写在集群
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalajm.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", noStationajm.count().toString) //无站点设备数
    ...
    val finalajm_fileDF = sqlContext.createDataFrame(rdd, schema)
       finalajm_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
         ...
         .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/fit_number")

     fitdistinctStationajm//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/fit_Station")

    /************************************************       需求2           *********************************************/
        /** 单个设备15分钟粒度的数据采集数 **/
        //ajm表: serverReceiveTimestamp, name, sex, nation, birth, addr, idno, authority, validBegin,
        // validEnd, face, capTime, deviceNum, deviceSn, gbNo, cameraOneFace, cameraTwoFace, deviceAddr
        val ajmRDD = distinctnull.rdd

        val single_ajmRDD = ajmRDD.map{x=>
        ...
        val single_ajmRDD_DF = single_ajmRDD.toDF()
        val single_ajmRDD_newNames = Seq( "date","gb_code","slot",  "number")
        val single_ajmRDD_renamed =single_ajmRDD_DF.toDF(single_ajmRDD_newNames: _*)

        //single_ajmRDD_renamed.show()
        single_ajmRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_15min_num")

        /** 所有设备15分钟粒度的数据采集数 **/
        val all_15_ajmRDD = ajmRDD.map{x=>
          val s = x.mkString("`").replace("[","").replace("]","").split("`")
          val date = tranTimeToString(s(0)).substring(0,10)
          val zeroTimestamp = date +" 00:00:00"
          val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
        ...
        val all_15_ajmRDD_DF = all_15_ajmRDD.toDF()
        val all_15_ajmRDD_newNames = Seq( "date","slot",  "number")
        val all_15_ajmRDD_renamed =all_15_ajmRDD_DF.toDF(all_15_ajmRDD_newNames: _*)
        //all_15_ajmRDD_renamed.show()
        all_15_ajmRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/all_15min_num")

       /** 所有设备每小时粒度的数据采集数 **/
        val all_60_ajmRDD = ajmRDD.map{x=>
          val s = x.mkString("`").replace("[","").replace("]","").split("`")
          val date = tranTimeToString(s(0)).substring(0,10)
          val zeroTimestamp = date +" 00:00:00"
        ...
        val all_60_ajmRDD_DF = all_60_ajmRDD.toDF()
        val all_60_ajmRDD_newNames = Seq( "date","slot",  "number")
        val all_60_ajmRDD_renamed =all_60_ajmRDD_DF.toDF(all_60_ajmRDD_newNames: _*)
        //all_60_ajmRDD_renamed.show()
        all_60_ajmRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/all_60min_num")

        /*******************************************需求3、4***************************************************/
        /** 单个设备分天计算延时分布 **/
        val single_parquetRDD = ajmRDD.map{x=>
          val s = x.mkString("`").replace("[","").replace("]","").split("`")
          val receiveTime = s(0)
          val dateTime = s(11)
        ...
        single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_delay")
       ...
       /*** 单个设备最小/最大/平均延时 ***/
        ...
        avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_avg_delay")
        //max_delayDF.show()
        max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_max_delay")
        //min_delayDF.show()
        min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_min_delay")

       /*** 所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10 ***/
        val all_delaygroupRDD = ajmRDD.map{x=>
          val s = x.mkString("`").replace("[","").replace("]","").split("`")
          val receiveTime = s(0)
          val dateTime = s(11)
          val date = dateTime.substring(0,10)
        ...
        delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/all_delay")
       ...
       /*** 所有设备延时日均值 ***/
        ...
        avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/all_avg_delay")

       /*** 单个设备分天计算发送频率 ***/
        val single_freqRDD = ajmRDD.map{x=>
          val s = x.mkString("`").replace("[","").replace("]","").split("`")
          val receiveTime = s(0)
          val dateTime = s(11)
          val date = dateTime.substring(0,10)
        ...
        dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_frequent")
...
/*        avg_freDF.show()
        ...
        avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_avg_frequent")

        max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_max_frequent")

        min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/single_min_frequent")

       /*** 所有设备分天所有频率 ***/
        val all_fregroupRDD = ajmRDD.map{x=>
          val s = x.mkString("`").replace("[","").replace("]","").split("`")
          val receiveTime = s(0)
          val dateTime = s(11)
          val date = dateTime.substring(0,10)
        ...
        //all_fregroupRDD.foreach(x=>println(x))
    ...
    all_fregroupRDD.isEmpty()
       ...
       /*** 所有设备分天所有频率分布 ***/
        ...
        val fre_newNames = Seq("date", "0-5秒","5-10秒","10-30秒","30-60秒","60-120秒","120-180秒","180-240秒","240-300秒",">300秒","∞秒")
        val fre_Renamed = all_freDF.toDF(fre_newNames: _*)
        //fre_Renamed.show()
        fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/all_frequent")
       ...
       /*** 所有设备频率日均值 ***/
        ...
        allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
          .write.mode(SaveMode.Overwrite)
          .format("com.databricks.spark.csv")
          .save("/user/sibat/GongAn_analyze/ajm_idcard/"+args(0)+"/all_avg_frequent")
  }
}
