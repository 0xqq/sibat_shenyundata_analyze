...
/*  case class St(gb_code: String, dev_id: String, dev_type: String, dev_name: String, dev_mac: String, dev_ip: String,
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("st_alarm_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("st_alarm_lzw")
      .getOrCreate()
...
/*    val deviceInfoDF = fileRDD.map { line => line.split("\\s{1}|\t")}
    ...
    //val parquetDFStorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/st_alarm/"+args(0))
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/st_alarm/"+args(0)
   ...
   |-- cameraId: string
   |-- cameraName: string
   |-- chaosDensity: integer
   |-- crowdDensity: integer
   |-- crowdNumberAllImage: integer
   |-- crowdNumberWithEvent: integer
   |-- dateTime: string
   |-- image: string
   |-- eventType: string
   |-- startTime: string
   ...
   |-- regionId: string*/
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/fit_number")

    //fitdistinctStationSt.show()             ////各站点匹配数
    fitdistinctStationSt//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/fit_Station")

    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/
    //     st_status表：serverReceiveTimestamp ，cameraId ,  cameraName  , dateTime  , intensity , peopleCount , safetyIndex  , gbNo
    //     st_alarm表：serverReceiveTimestamp ,   cameraId  ,  cameraName  ,chaosDensity , crowdDensity , crowdNumberAllImage ,
    //    crowdNumberWithEvent ,   dateTime   ,   image ,   eventType  , startTime  , endTime , lastTime  ,  gbNo , regionId
    ...
    single_StRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_15min_num")
    ...
    all_15_StRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/all_15min_num")
    ...
    all_60_StRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/all_60min_num")
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_delay")
...
/*         avg_delayDF.show()
    ...
    avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_min_delay")
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/all_avg_delay")
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_frequent")
...
/*     avg_freDF.show()
    ...
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/single_min_frequent")
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/all_frequent")
    ...
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/st_alarm/"+args(0)+"/all_avg_frequent")
  }
}
