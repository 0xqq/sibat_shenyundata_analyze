...
/*  case class basicDev(gb_code: String, dev_id: String, dev_type: String, dev_name: String, dev_mac: String, dev_ip: String,
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("rzx_location_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("rzx_location_lzw")
      .getOrCreate()
...
/*    val deviceInfoDF = fileRDD.map { line => line.split("\\s{1}|\t")}
    ...
    //val parquetDFrzx_locationorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/rzx_location/"+args(0)+"/part*parquet")
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/rzx_location/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFrzx_locationorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        if(file_path.substring(file_path.size-8,file_path.size)==".parquet"){
          parquetDFrzx_locationorg = sqlContext.read.parquet(file_path)
        }
      } catch {
   ...
   |-- SERVICE_TYPE: integer (nullable = true)
   ...
   |-- gbNo: string (nullable = true)*/
    ...
    /***********************************************************              需求1                *****************************************************************/
    val originnum = parquetDFrzx_locationorg.count()
    val parquetDFrzx_location = parquetDFrzx_locationorg.distinct().dropDuplicates()
    val distinctnum =parquetDFrzx_location.count()

    parquetDFrzx_location.filter(parquetDFrzx_location("gbNo")=!="").createOrReplaceTempView("rzx_location_table")
    val distinctnull = sqlContext.sql("select * from rzx_location_table")

    val onlinenum = sqlContext.sql("select distinct gbNo from rzx_location_table")
    val finalrzx_location = sqlContext.sql("select distinct gbNo from rzx_location_table,deviceInfo_table where rzx_location_table.gbNo=deviceInfo_table.gb_code ")
    val noStationrzx_location = sqlContext.sql("select distinct gbNo from rzx_location_table,deviceInfo_table where rzx_location_table.gbNo=deviceInfo_table.gb_code and deviceInfo_table.station_name=''")
    val fitdistinctStationrzx_location = sqlContext.sql("select station_name,count(distinct gbNo) from rzx_location_table,deviceInfo_table where rzx_location_table.gbNo=deviceInfo_table.gb_code group by station_name")
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalrzx_location.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", finalrzx_location.count().toString) //无站点设备数
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/fit_number")

    //fitdistinctStationrzx_location.show()             ////各站点匹配数
    fitdistinctStationrzx_location//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/fit_Station")

    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/

    val rzx_locationRDD = distinctnull.rdd

    val single_rzx_locationRDD = rzx_locationRDD.map{x=>
    ...
    val single_rzx_locationRDD_DF = single_rzx_locationRDD.toDF()
    val single_rzx_locationRDD_newNames = Seq( "date","gb_code","slot",  "number")
    val single_rzx_locationRDD_renamed =single_rzx_locationRDD_DF.toDF(single_rzx_locationRDD_newNames: _*)
    //single_rzx_locationRDD_renamed.show()
    single_rzx_locationRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_15min_num")

    /** 所有设备15分钟粒度的数据采集数 **/
    val all_15_rzx_locationRDD = rzx_locationRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
      val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
    ...
    val all_15_rzx_locationRDD_DF = all_15_rzx_locationRDD.toDF()
    val all_15_rzx_locationRDD_newNames = Seq( "date","slot",  "number")
    val all_15_rzx_locationRDD_renamed =all_15_rzx_locationRDD_DF.toDF(all_15_rzx_locationRDD_newNames: _*)
    //all_15_rzx_locationRDD_renamed.show()
    all_15_rzx_locationRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/all_15min_num")

    /** 所有设备每小时粒度的数据采集数 **/
    val all_60_rzx_locationRDD = rzx_locationRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
    ...
    val all_60_rzx_locationRDD_DF = all_60_rzx_locationRDD.toDF()
    val all_60_rzx_locationRDD_newNames = Seq( "date","slot",  "number")
    val all_60_rzx_locationRDD_renamed =all_60_rzx_locationRDD_DF.toDF(all_60_rzx_locationRDD_newNames: _*)
    //all_60_rzx_locationRDD_renamed.show()
    all_60_rzx_locationRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/all_60min_num")

    /********************************************需求3、4****************************************************/
    /** 单个设备分天计算延时分布 **/
   /* val single_parquetRDD = rzx_locationRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val dateTime = receiveTime //数据中没有datetime
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_delay")
...
/*      avg_delayDF.show()
   ...
   /* avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_min_delay")

    //所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10
    val all_delaygroupRDD = rzx_locationRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val dateTime = receiveTime //数据中没有datetime
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/all_avg_delay")*/

    /*** 单个设备分天计算发送频率 ***/
    val single_freqRDD = rzx_locationRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val gbNo = s(10)
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_frequent")
...
*/
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/single_min_frequent")


    /*** 所有设备分天所有频率 ***/
    val all_fregroupRDD = rzx_locationRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val gbNo = s(10)
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/all_frequent")
    ...
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_location/"+args(0)+"/all_avg_frequent")
  }
}
