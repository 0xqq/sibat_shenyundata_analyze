...
/*  case class basicDev(gb_code: String, dev_id: String, dev_type: String, dev_name: String, dev_mac: String, dev_ip: String,
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("rzx_device_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("rzx_device_lzw")
      .getOrCreate()
...
/*    val deviceInfoDF = fileRDD.map { line => line.split("\\s{1}|\t")}
    ...
    //val parquetDFrzx_deviceorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/rzx_device/"+args(0))
    //读取文件时对异常文件处理
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/rzx_device/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFrzx_deviceorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        parquetDFrzx_deviceorg = sqlContext.read.parquet(file_path)
      } catch {
   ...
   |-- EQUIPMENT_NUM: string (nullable = true)
   |-- EQUIPMENT_NAME: string (nullable = true)
   |-- MAC: string (nullable = true)
   |-- SERVICE_CODE: string (nullable = true)
   |-- PROVINCE_CODE: string (nullable = true)
   |-- CITY_CODE: string (nullable = true)
   |-- AREA_CODE: string (nullable = true)
   |-- EQUIPMENT_CODE: string (nullable = true)
   |-- LONGITUDE: string (nullable = true)
   |-- LATITUDE: string (nullable = true)
   |-- SUBWAY_STATION: string (nullable = true)
   |-- SUBWAY_VEHICLE_INFO: string (nullable = true)
   |-- SUBWAY_LINE_INFO: string (nullable = true)
   |-- SUBWAY_COMPARTMENT_NUM: string (nullable = true)
   |-- CAR_CODE: string (nullable = true)
   |-- UPLOAD_TIME_INTERVAL: long (nullable = true)
   |-- COLLECTION_RADIUS: long (nullable = true)
   |-- CREATE_TIME: string (nullable = true)
   |-- gbNo: string (nullable = true)*/

    /******************************************************************            需求1          *****************************************************************/
    val originnum = parquetDFrzx_deviceorg.count()
    val parquetDFrzx_device= parquetDFrzx_deviceorg.distinct().dropDuplicates()
    val distinctnum =parquetDFrzx_device.count()

    parquetDFrzx_device.filter(parquetDFrzx_device("gbNo")=!="").createOrReplaceTempView("rzx_device_table")
    val distinctnull = sqlContext.sql("select serverReceiveTimestamp,CREATE_TIME,gbNo from rzx_device_table")

    val distinctnullnum = distinctnull.count()
    val onlinenum = sqlContext.sql("select distinct gbNo from rzx_device_table")
    val finalrzx_device = sqlContext.sql("select distinct gbNo from rzx_device_table,deviceInfo_table where rzx_device_table.gbNo=deviceInfo_table.gb_code")
    val noStationrzx_device = sqlContext.sql("select distinct gbNo from rzx_device_table,deviceInfo_table where rzx_device_table.gbNo=deviceInfo_table.gb_code and deviceInfo_table.station_name=''")
    val fitdistinctStationrzx_device = sqlContext.sql("select station_name,count(distinct gbNo) from rzx_device_table, deviceInfo_table where rzx_device_table.gbNo= deviceInfo_table.gb_code group by station_name")
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalrzx_device.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", noStationrzx_device.count().toString) //无站点设备数
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/fit_number")


    //fitdistinctStationrzx_device.show()             ////各站点匹配数
    fitdistinctStationrzx_device//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/fit_Station")

    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/

    val rzx_deviceRDD = distinctnull.rdd

    val single_rzx_deviceRDD = rzx_deviceRDD.map{x=>
    ...
    val single_rzx_deviceRDD_DF = single_rzx_deviceRDD.toDF()
    val single_rzx_deviceRDD_newNames = Seq( "date","gb_code","slot",  "number")
    val single_rzx_deviceRDD_renamed =single_rzx_deviceRDD_DF.toDF(single_rzx_deviceRDD_newNames: _*)
    //single_rzx_deviceRDD_renamed.show()
    single_rzx_deviceRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_15min_num")

    /** 所有设备15分钟粒度的数据采集数 **/
    val all_15_rzx_deviceRDD = rzx_deviceRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
      val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
    ...
    val all_15_rzx_deviceRDD_DF = all_15_rzx_deviceRDD.toDF()
    val all_15_rzx_deviceRDD_newNames = Seq( "date","slot",  "number")
    val all_15_rzx_deviceRDD_renamed =all_15_rzx_deviceRDD_DF.toDF(all_15_rzx_deviceRDD_newNames: _*)
    //all_15_rzx_deviceRDD_renamed.show()
    all_15_rzx_deviceRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/all_15min_num")

    /** 所有设备每小时粒度的数据采集数 **/
    val all_60_rzx_deviceRDD = rzx_deviceRDD.map{x=>
      val s = x.mkString(":").replace("[","").replace("]","").split(":")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
    ...
    val all_60_rzx_deviceRDD_DF = all_60_rzx_deviceRDD.toDF()
    val all_60_rzx_deviceRDD_newNames = Seq( "date","slot",  "number")
    val all_60_rzx_deviceRDD_renamed =all_60_rzx_deviceRDD_DF.toDF(all_60_rzx_deviceRDD_newNames: _*)
    //all_60_rzx_deviceRDD_renamed.show()
    all_60_rzx_deviceRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/all_60min_num")

    /********************************************需求3、4****************************************************/
    /** 单个设备分天计算延时分布 **/
    val single_parquetRDD = rzx_deviceRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(1).substring(0,19)
      //20180624的CREATE_TIME字段异常需作如下处理
//      val left = s(18).substring(0,10)
//      val right1 = s(18).substring(11,13)
//      val right2 = s(18).substring(13,15)
//      val right3 = s(18).substring(15,17)
//      val dateTime = s"$left $right1:$right2:$right3"
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_delay")
    ...
    avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_min_delay")

    /*** 所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10 ***/
    val all_delaygroupRDD = rzx_deviceRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(1).substring(0,19)
//      val right1 = s(18).substring(11,13)
...
//      val dateTime = s"$left $right1:$right2:$right3"
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/all_avg_delay")

    /*** 单个设备分天计算发送频率 ***/
    val single_freqRDD = rzx_deviceRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = s(1).substring(0,10)
      val gbNo = s(2)
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_frequent")
    ...
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/single_min_frequent")

    /*** 所有设备分天所有频率 ***/
    val all_fregroupRDD = rzx_deviceRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = s(1).substring(0,10)
      val gbNo = s(2)
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/all_frequent")
    ...
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_device/"+args(0)+"/all_avg_frequent")
  }
}
