...
/*  case class basicDev(gb_code: String, dev_id: String, dev_type: String, dev_name: String, dev_mac: String, dev_ip: String,
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("sensordoor_idcard_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("sensordoor_idcard_lzw")
...
/*    val deviceInfoDF = fileRDD.map { line => line.split("\\s{1}|\t")}
    ...
    //val parquetDFsensordoor_idcardorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/sensordoor_idcard/"+args(0))
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/sensordoor_idcard/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFsensordoor_idcardorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        parquetDFsensordoor_idcardorg = sqlContext.read.parquet(file_path)
      } catch {
   ...
   |-- validBegin: string (nullable = true)
   |-- validEnd: string (nullable = true)*/

    /******************************** *****************                                 需求1                *****************************************************************/
    val originnum = parquetDFsensordoor_idcardorg.count()
    val parquetDFsensordoor_idcard = parquetDFsensordoor_idcardorg.distinct().dropDuplicates()
    val distinctnum =parquetDFsensordoor_idcard.count()

    parquetDFsensordoor_idcard.filter(parquetDFsensordoor_idcard("gbNo")=!="").createOrReplaceTempView("sensordoor_idcard_table")
    val distinctnull = sqlContext.sql("select * from sensordoor_idcard_table")

    val onlinenum = sqlContext.sql("select distinct gbNo from sensordoor_idcard_table")
    val finalsensordoor_idcard = sqlContext.sql("select distinct gbNo from sensordoor_idcard_table ,  deviceInfo_table where sensordoor_idcard_table.gbNo = deviceInfo_table.gb_code ")
    val noStationsensordoor_idcard = sqlContext.sql("select distinct gbNo from sensordoor_idcard_table ,  deviceInfo_table where  sensordoor_idcard_table.gbNo = deviceInfo_table.gb_code and  deviceInfo_table.station_name=''")
    val fitdistinctStationsensordoor_idcard = sqlContext.sql("select station_name,count(distinct gbNo) from sensordoor_idcard_table, deviceInfo_table where sensordoor_idcard_table.gbNo = deviceInfo_table.gb_code group by station_name")
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalsensordoor_idcard.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", noStationsensordoor_idcard.count().toString) //无站点设备数
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/fit_number")

    //fitdistinctStationsensordoor_idcard.show()             ////各站点匹配数
    fitdistinctStationsensordoor_idcard//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/fit_Station")


    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/
    //sensordoor_idcard表: serverReceiveTimestamp, name, sex, nation, birth, addr, idno, authority, validBegin,
    // validEnd, face, capTime, deviceNum, deviceSn, gbNo, cameraOneFace, cameraTwoFace, deviceAddr
    val sensordoor_idcardRDD = distinctnull.rdd

    val single_sensordoor_idcardRDD = sensordoor_idcardRDD.map{x=>
    ...
    val single_sensordoor_idcardRDD_DF = single_sensordoor_idcardRDD.toDF()
    val single_sensordoor_idcardRDD_newNames = Seq( "date","gb_code","slot",  "number")
    val single_sensordoor_idcardRDD_renamed =single_sensordoor_idcardRDD_DF.toDF(single_sensordoor_idcardRDD_newNames: _*)
   // single_sensordoor_idcardRDD_renamed.show()
    single_sensordoor_idcardRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_15min_num")

    /** 所有设备15分钟粒度的数据采集数 **/
    val all_15_sensordoor_idcardRDD = sensordoor_idcardRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
      val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
    ...
    val all_15_sensordoor_idcardRDD_DF = all_15_sensordoor_idcardRDD.toDF()
    val all_15_sensordoor_idcardRDD_newNames = Seq( "date","slot",  "number")
    val all_15_sensordoor_idcardRDD_renamed =all_15_sensordoor_idcardRDD_DF.toDF(all_15_sensordoor_idcardRDD_newNames: _*)
    //all_15_sensordoor_idcardRDD_renamed.show()
    all_15_sensordoor_idcardRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/all_15min_num")

    /** 所有设备每小时粒度的数据采集数 **/
    val all_60_sensordoor_idcardRDD = sensordoor_idcardRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
    ...
    val all_60_sensordoor_idcardRDD_DF = all_60_sensordoor_idcardRDD.toDF()
    val all_60_sensordoor_idcardRDD_newNames = Seq( "date","slot",  "number")
    val all_60_sensordoor_idcardRDD_renamed =all_60_sensordoor_idcardRDD_DF.toDF(all_60_sensordoor_idcardRDD_newNames: _*)
    //all_60_sensordoor_idcardRDD_renamed.show()
    all_60_sensordoor_idcardRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/all_60min_num")

    /********************************************需求3、4****************************************************/
    /** 单个设备分天计算延时分布 **/
    val single_parquetRDD = sensordoor_idcardRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(4)
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_delay")
...
/*       avg_delayDF.show()
    ...
    avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_min_delay")

    /*** 所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10 ***/
    val all_delaygroupRDD = sensordoor_idcardRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(4)
      val date = dateTime.substring(0,10)
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/all_avg_delay")

    /*** 单个设备分天计算发送频率 ***/
    val single_freqRDD = sensordoor_idcardRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(4)
      val date = dateTime.substring(0,10)
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_frequent")
...
/*    avg_freDF.show()
    ...
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/single_min_frequent")

    /*** 所有设备分天所有频率 ***/
    val all_fregroupRDD = sensordoor_idcardRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(4)
      val date = dateTime.substring(0,10)
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/all_frequent")
    ...
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/sensordoor_idcard/"+args(0)+"/all_avg_frequent")
  }
}
