...
/*  case class basicDev(gb_code: String, dev_id: String, dev_type: String, dev_name: String, dev_mac: String, dev_ip: String,
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("ty_imsi_lzw")//.setMaster("local")
...
/*    val deviceInfoDF = fileRDD.map { line => line.split("\\s{1}|\t")}
    ...
    //val parquetDFty_imsiorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/ty_imsi/"+args(0))
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/ty_imsi/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFty_imsiorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        parquetDFty_imsiorg = sqlContext.read.parquet(file_path)
      } catch {
   ...
   |-- location: string*/
    ...
    /*************************************************           需求1              *****************************************************************/
    val originnum = parquetDFty_imsiorg.count()
    val parquetDFty_imsi = parquetDFty_imsiorg.distinct().dropDuplicates()
    val distinctnum =parquetDFty_imsi.count()

    parquetDFty_imsi.filter(parquetDFty_imsi("gbNo")=!="").createOrReplaceTempView("ty_imsi_table")
    val distinctnull = sqlContext.sql("select * from ty_imsi_table")

    val onlinenum = sqlContext.sql("select distinct gbNo from ty_imsi_table")
    val finalty_imsi = sqlContext.sql("select distinct gbNo from ty_imsi_table,deviceInfo_table where ty_imsi_table.gbNo=deviceInfo_table.gb_code")
    val noStationty_imsi = sqlContext.sql("select distinct gbNo from ty_imsi_table,deviceInfo_table where ty_imsi_table.gbNo=deviceInfo_table.gb_code and deviceInfo_table.station_name=''")
    val fitdistinctStationty_imsi = sqlContext.sql("select station_name,count(distinct gbNo) from ty_imsi_table, deviceInfo_table where ty_imsi_table.gbNo=deviceInfo_table.gb_code group by station_name")
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalty_imsi.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", noStationty_imsi.count().toString) //无站点设备数
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/fit_number")

    //fitdistinctStationty_imsi.show()             ////各站点匹配数
    fitdistinctStationty_imsi//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/fit_Station")

    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/

    val ty_imsiRDD = distinctnull.rdd

    val single_ty_imsiRDD = ty_imsiRDD.map{x=>
    ...
    val single_ty_imsiRDD_DF = single_ty_imsiRDD.toDF()
    val single_ty_imsiRDD_newNames = Seq( "date","gb_code","slot",  "number")
    val single_ty_imsiRDD_renamed =single_ty_imsiRDD_DF.toDF(single_ty_imsiRDD_newNames: _*)
    //single_ty_imsiRDD_renamed.show()
    single_ty_imsiRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_15min_num")

    /** 所有设备15分钟粒度的数据采集数 **/
    val all_15_ty_imsiRDD = ty_imsiRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
      val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
    ...
    val all_15_ty_imsiRDD_DF = all_15_ty_imsiRDD.toDF()
    val all_15_ty_imsiRDD_newNames = Seq( "date","slot",  "number")
    val all_15_ty_imsiRDD_renamed =all_15_ty_imsiRDD_DF.toDF(all_15_ty_imsiRDD_newNames: _*)
    //all_15_ty_imsiRDD_renamed.show()
    all_15_ty_imsiRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/all_15min_num")

    /** 所有设备每小时粒度的数据采集数 **/
    val all_60_ty_imsiRDD = ty_imsiRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
    ...
    val all_60_ty_imsiRDD_DF = all_60_ty_imsiRDD.toDF()
    val all_60_ty_imsiRDD_newNames = Seq( "date","slot",  "number")
    val all_60_ty_imsiRDD_renamed =all_60_ty_imsiRDD_DF.toDF(all_60_ty_imsiRDD_newNames: _*)
    //all_60_ty_imsiRDD_renamed.show()
    all_60_ty_imsiRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/all_60min_num")

    /********************************************需求3、4****************************************************/
    /** 单个设备分天计算延时分布 **/
    val single_parquetRDD = ty_imsiRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(3)
      val date = tranTimeToString(s(3)).substring(0,10)
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_delay")
...
*/
    ...
    avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_min_delay")

    /*** 所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10 ***/
    val all_delaygroupRDD = ty_imsiRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(3)
      val date = tranTimeToString(s(3)).substring(0,10)
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/all_avg_delay")

    /*** 单个设备分天计算发送频率 ***/
    val single_freqRDD = ty_imsiRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(3)
      val date = tranTimeToString(s(3)).substring(0,10)
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_frequent")
...
/*       avg_freDF.show()
    ...
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/single_min_frequent")

    /*** 所有设备分天所有频率 ***/
    val all_fregroupRDD = ty_imsiRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(3)
      val date = tranTimeToString(s(3)).substring(0,10)
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/all_frequent")
    ...
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ty_imsi/"+args(0)+"/all_avg_frequent")
  }
}
