...
object rzx_feature_lzw {
  ...
  /** 计算延时 **/
  def subtract(datetime:Long,receivetime:Long):Int={
    val diff = receivetime-datetime
    return diff.toInt.abs
  }
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("rzx_feature_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("rzx_feature_lzw")
      .getOrCreate()

    /** 读文件basic_device_info.txt **/
    //val fileRDD = sqlContext.sparkContext.textFile("/user/hadoop/GongAnV2/basic_device_info.txt")
    val fileRDD = sqlContext.sparkContext.textFile("/user/sibat/GongAn_analyze/basic_device_info.txt")
    //val fileRDD = sqlContext.sparkContext.textFile("/user/zhangjun/Data/gongan/basic_device_info.txt")
    ...
    val deviceInfoDF = fileRDD.map { line => line.split("\\s{1}|\t")}
      ...
      val notnulldeviceInfoDF = deviceInfoDF.filter(deviceInfoDF("gb_code")=!="")
    //注册一个临时表
    notnulldeviceInfoDF.createOrReplaceTempView("deviceInfo_table")

    //val parquetDFrzx_featureorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/rzx_feature/"+args(0))
    //val parquetDFrzx_featureorg =  sqlContext.read.parquet("/user/zhangjing/gongan_hate/rzxfeature20180812/"+args(0))
    //val parquetDFrzx_featureorg =  sqlContext.read.parquet("/user/zhangjing/gongan_hate/rzx_feature_part/"+args(0))
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/rzx_feature/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFrzx_featureorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        parquetDFrzx_featureorg = sqlContext.read.parquet(file_path)
      } catch {
   ...
   |-- AP_CHANNEL: string (nullable = true)
   |-- AP_ENCRYTYPE: string (nullable = true)
   |-- BSSID: string (nullable = true)
   |-- COMPANY_ID: string (nullable = true)
   |-- CONSULT_XPOINT: string (nullable = true)
   |-- CONSULT_YPOINT: string (nullable = true)
   |-- DEVICENUM: string (nullable = true)
   |-- DEVMAC: string (nullable = true)
   |-- END_TIME: long (nullable = true)
   |-- ESSID: string (nullable = true)
   |-- HISTORY_ESSID: string (nullable = true)
   ...
   |-- XPOINT: string (nullable = true)
   |-- YPOINT: string (nullable = true)
   |-- gbNo: string (nullable = true)*/

    /***********************************************                        需求1                *****************************************************************/
    val originnum = parquetDFrzx_featureorg.count()
    val parquetDFrzx_feature = parquetDFrzx_featureorg.distinct().dropDuplicates()
    val distinctnum =parquetDFrzx_feature.count()

    parquetDFrzx_feature.filter(parquetDFrzx_feature("gbNo")=!="").createOrReplaceTempView("rzx_feature_table")
    val distinctnull = sqlContext.sql("select serverReceiveTimestamp,START_TIME,gbNo from rzx_feature_table")/* where gbNo !=''*/

    val onlinenum = sqlContext.sql("select distinct gbNo from rzx_feature_table")
    val finalrzx_feature = sqlContext.sql("select distinct gbNo from rzx_feature_table,deviceInfo_table where rzx_feature_table.gbNo=deviceInfo_table.gb_code")
    val noStationrzx_feature = sqlContext.sql("select distinct gbNo from rzx_feature_table,deviceInfo_table where rzx_feature_table.gbNo=deviceInfo_table.gb_code and deviceInfo_table.station_name=''")
    val fitdistinctStationrzx_feature = sqlContext.sql("select station_name,count(distinct gbNo) from rzx_feature_table,deviceInfo_table where rzx_feature_table.gbNo=deviceInfo_table.gb_code group by station_name")
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalrzx_feature.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", noStationrzx_feature.count().toString) //无站点设备数
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/fit_number")


   //fitdistinctStationrzx_feature.show()             ////各站点匹配数
    fitdistinctStationrzx_feature//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/fit_Station")

    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/
    val rzx_featureRDD = distinctnull.rdd//.filter(_.size==3)

    val single_rzx_featureRDD = rzx_featureRDD.map{x=>
    ...
    val single_rzx_featureRDD_DF = single_rzx_featureRDD.toDF()
    val single_rzx_featureRDD_newNames = Seq( "date","gb_code","slot",  "number")
    val single_rzx_featureRDD_renamed =single_rzx_featureRDD_DF.toDF(single_rzx_featureRDD_newNames: _*)
    //single_rzx_featureRDD_renamed.show()
    single_rzx_featureRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_15min_num")

    /** 所有设备15分钟粒度的数据采集数 **/
    val all_15_rzx_featureRDD = rzx_featureRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
      val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
    ...
    val all_15_rzx_featureRDD_DF = all_15_rzx_featureRDD.toDF()
    val all_15_rzx_featureRDD_newNames = Seq( "date","slot",  "number")
    val all_15_rzx_featureRDD_renamed =all_15_rzx_featureRDD_DF.toDF(all_15_rzx_featureRDD_newNames: _*)
    //all_15_rzx_featureRDD_renamed.show()
    all_15_rzx_featureRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/all_15min_num")

    /** 所有设备每小时粒度的数据采集数 **/
    val all_60_rzx_featureRDD = rzx_featureRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
    ...
    val all_60_rzx_featureRDD_DF = all_60_rzx_featureRDD.toDF()
    val all_60_rzx_featureRDD_newNames = Seq( "date","slot",  "number")
    val all_60_rzx_featureRDD_renamed =all_60_rzx_featureRDD_DF.toDF(all_60_rzx_featureRDD_newNames: _*)
    //all_60_rzx_featureRDD_renamed.show()
    all_60_rzx_featureRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/all_60min_num")

    /********************************************需求3、4****************************************************/
    /** 单个设备分天计算延时分布 **/
    val single_parquetRDD = rzx_featureRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(1)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val sub = subtract(dateTime.toLong,receiveTime.toLong)
      val gbNo = s(2)
      ((gbNo,date),sub)
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_delay")
...
/*        avg_delayDF.show()
    ...
    avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_min_delay")

    /*** 所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10 ***/
    val all_delaygroupRDD = rzx_featureRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = s(1)
      val date = tranTimeToString(receiveTime).substring(0,10)
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/all_avg_delay")

    /*** 单个设备分天计算发送频率 ***/
    val single_freqRDD = rzx_featureRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val gbNo = s(2)
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_frequent")
...
/*        avg_freDF.show()
    ...
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/single_min_frequent")

    /*** 所有设备分天所有频率 ***/
    val all_fregroupRDD = rzx_featureRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val date = tranTimeToString(receiveTime).substring(0,10)
      val gbNo = s(2)
    ...
    }.groupByKey().map(x=>(x._1,x._2.flatten.mkString(",")))
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/all_frequent")

    /*** 所有设备频率日均值 ***/
    val allfre_avg = Seq("date","avg")
    val allfre_avgRDD = all_fregroupRDD.map(x=>(x._1,x._2.split(",").map(x1=>
      if (x1==""){//若该天只有一条数据则返回NaN值
        (0,0)
      }else{
        (x1.toInt,1)}
    ).reduce((x,y)=>(x._1+y._1,x._2+y._2))))
      .map(x=>(x._1,(x._2._1.toDouble/x._2._2.toDouble).formatted("%.2f")))
    val allavg_freDF =allfre_avgRDD.toDF(allfre_avg:_*)
    allavg_freDF.show()
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/rzx_feature/"+args(0)+"/all_avg_frequent")
  }
}
