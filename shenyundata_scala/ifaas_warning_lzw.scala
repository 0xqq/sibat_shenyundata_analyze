...
object ifaas_warning_lzw {
  ...
  def main(args: Array[String]): Unit = {
    //val conf = new SparkConf().setAppName("ifaas_warning_lzw")//.setMaster("local")
    //val sc = new SparkContext(conf)
    //sc.setLogLevel("ERROR")

    val sqlContext = SparkSession
      .builder()
      .appName("ifaas_warning_lzw")
      .getOrCreate()
    ...
    //val parquetDFifaas_warningorg =  sqlContext.read.parquet("/user/hadoop/GongAnV2/ifaas_warning/"+args(0))
    //读取文件时对异常文件处理
    ...
    val hdfsPath = "/user/hadoop/GongAnV2/ifaas_warning/"+args(0)
    val paths : List[String] = listChildren(hdfs, hdfsPath, holder).toList

    var parquetDFifaas_warningorg:sql.DataFrame = null
    for (file_path<-paths){
      try{
        parquetDFifaas_warningorg = sqlContext.read.parquet(file_path)
      } catch {
   ...
   |-- alarmId: long (nullable = true)
   |-- personId: long (nullable = true)
   |-- repoId: long (nullable = true)
   |-- taskId: long (nullable = true)
   |-- blackId: long (nullable = true)
   |-- faceId: long (nullable = true)
   |-- confidence: double (nullable = true)
   |-- timestamp: long (nullable = true)
   |-- alarmType: integer (nullable = true)
   |-- serverId: long (nullable = true)
   |-- realName: string (nullable = true)
   |-- idcardNo: string (nullable = true)
   |-- capturePic: string (nullable = true)
   |-- thumbPic: string (nullable = true)
   |-- blackPic: string (nullable = true)
   |-- cameraId: long (nullable = true)*/

    /******************************** *****************                                 需求1                *****************************************************************/
    val originnum = parquetDFifaas_warningorg.count()
    val parquetDFifaas_warning = parquetDFifaas_warningorg.distinct().dropDuplicates()
    val distinctnum =parquetDFifaas_warning.count()

    parquetDFifaas_warning.filter(parquetDFifaas_warning("gbNo")=!="").createOrReplaceTempView("ifaas_warning_table")
    val distinctnull = sqlContext.sql("select serverReceiveTimestamp,gbNo,timestamp from ifaas_warning_table")

    val distinctnullnum = distinctnull.count()
    val onlinenum = sqlContext.sql("select distinct gbNo from ifaas_warning_table")
    val finalifaas_warning = sqlContext.sql("select distinct gbNo from ifaas_warning_table,deviceInfo_table where ifaas_warning_table.gbNo=deviceInfo_table.gb_code")
    val noStationifaas_warning = sqlContext.sql("select distinct gbNo from ifaas_warning_table,deviceInfo_table where ifaas_warning_table.gbNo=deviceInfo_table.gb_code and deviceInfo_table.station_name=''")
    val fitdistinctStationifaas_warning = sqlContext.sql("select station_name,count(distinct gbNo) from ifaas_warning_table, deviceInfo_table where ifaas_warning_table.gbNo=deviceInfo_table.gb_code group by station_name")
    ...
    val rdd = sqlContext.sparkContext.parallelize(Seq(
      Row("Total number of matches", finalifaas_warning.count().toString),//全网匹配数
      ...
      Row("Number of devices without site", noStationifaas_warning.count().toString) //无站点设备数
    ...
    final_fileDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      ...
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/fit_number")

    //fitdistinctStationifaas_warning.show()             ////各站点匹配数
    fitdistinctStationifaas_warning//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/fit_Station")

    /***********************************                                需求2                                    ********************************/
    /** 单个设备15分钟粒度的数据采集数 **/
    //ifaas_warning表:serverReceiveTimestamp                  gbNo            alarmId  personId  ...
    val ifaas_warningRDD = distinctnull.rdd

    val single_ifaas_warningRDD = ifaas_warningRDD.map{x=>
    ...
    val single_ifaas_warningRDD_DF = single_ifaas_warningRDD.toDF()
    val single_ifaas_warningRDD_newNames = Seq( "date","gb_code","slot",  "number")
    val single_ifaas_warningRDD_renamed =single_ifaas_warningRDD_DF.toDF(single_ifaas_warningRDD_newNames: _*)
    //single_ifaas_warningRDD_renamed.show()
    single_ifaas_warningRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_15min_num")

    /** 所有设备15分钟粒度的数据采集数 **/
    val all_15_ifaas_warningRDD = ifaas_warningRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
      val slot = (s(0).toLong - tranTimeToLong(zeroTimestamp))/900
    ...
    val all_15_ifaas_warningRDD_DF = all_15_ifaas_warningRDD.toDF()
    val all_15_ifaas_warningRDD_newNames = Seq( "date","slot",  "number")
    val all_15_ifaas_warningRDD_renamed =all_15_ifaas_warningRDD_DF.toDF(all_15_ifaas_warningRDD_newNames: _*)
    //all_15_ifaas_warningRDD_renamed.show()
    all_15_ifaas_warningRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/all_15min_num")

    /** 所有设备每小时粒度的数据采集数 **/
    val all_60_ifaas_warningRDD = ifaas_warningRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val date = tranTimeToString(s(0)).substring(0,10)
      val zeroTimestamp = date +" 00:00:00"
    ...
    val all_60_ifaas_warningRDD_DF = all_60_ifaas_warningRDD.toDF()
    val all_60_ifaas_warningRDD_newNames = Seq( "date","slot",  "number")
    val all_60_ifaas_warningRDD_renamed =all_60_ifaas_warningRDD_DF.toDF(all_60_ifaas_warningRDD_newNames: _*)
    //all_60_ifaas_warningRDD_renamed.show()
    all_60_ifaas_warningRDD_renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/all_60min_num")

    /********************************************需求3、4****************************************************/
    /** 单个设备分天计算延时分布 **/
    val single_parquetRDD = ifaas_warningRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
    ...
    single_delay//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_delay")
...
/*  avg_delayDF.show()
    ...
    avg_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_avg_delay")

    max_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_max_delay")

    min_delayDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_min_delay")

    /*** 所有设备分天计算分布;或用case实现.(2018-07-16，<0秒):10 ***/
    val all_delaygroupRDD = ifaas_warningRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = tranTimeToString(s(2))
      val date = dateTime.substring(0,10)
    ...
    delay_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/all_delay")
    ...
    avg_dealyDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/all_avg_delay")

    /*** 单个设备分天计算发送频率 ***/
    val single_freqRDD = ifaas_warningRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = tranTimeToString(s(2))
      val date = dateTime.substring(0,10)
    ...
    dfRenamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_frequent")
...
/*    avg_freDF.show()
    ...
    avg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_avg_frequent")

    max_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_max_frequent")

    min_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/single_min_frequent")

    /*** 所有设备分天所有频率 ***/
    val all_fregroupRDD = ifaas_warningRDD.map{x=>
      val s = x.mkString("`").replace("[","").replace("]","").split("`")
      val receiveTime = s(0)
      val dateTime = tranTimeToString(s(2))
      val date = dateTime.substring(0,10)
    ...
    fre_Renamed//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/all_frequent")
    ...
    allavg_freDF//.coalesce(1)  //设置为一个partition, 这样可以把输出文件合并成一个文件
      .write.mode(SaveMode.Overwrite)
      .format("com.databricks.spark.csv")
      .save("/user/sibat/GongAn_analyze/ifaas_warning/"+args(0)+"/all_avg_frequent")
  }
}
